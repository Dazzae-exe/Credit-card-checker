...
const validateCred = (array) => {
  //set up for card numbers that are < 16 char
  let digitCheck = 0;
  let sum = 0;
  let newArr = [];
  ...
  for (let i = array.length - 1; i >= 0; i--) {
    digitCheck++;
    //increments at the beginning of each loop, used instead of i % 2 to solve the < 16 numbers problem
    // Luhn algoritm takes place
    if (digitCheck % 2 === 0) {
      if (array[i] * 2 > 9) {
